...
def transform_to_allowed_gates(circuit, **kwargs):
    ...
    for node in dag.op_nodes():
        if node.name == "ry":
            ...
            # substitute gates
            if abs(angle % (2*np.pi) - 0) < threshold:
                dag.remove_op_node(node)
            elif abs(angle% (2*np.pi) - np.pi/2) < threshold:
            ...
            elif abs(angle % (2*np.pi) - np.pi) < threshold:
                qc_loc = QuantumCircuit(1)
                qc_loc.y(0)
                qc_loc_instr = qc_loc.to_instruction()
                dag.substitute_node(node, qc_loc_instr, inplace=True)
            elif abs(angle % (2*np.pi) - 1.5*np.pi) < threshold:
                qc_loc = QuantumCircuit(1)
                qc_loc.sdg(0)
                qc_loc.sxdg(0)
                qc_loc.s(0)
